...
const APP_VERSION = "v16.0 Obsidian Prime";
...
// Global AI Studio Handoffs
// The interface declaration is updated to use the existing AIStudio type 
// to avoid "Subsequent property declarations must have the same type" errors.
declare global {
  interface Window {
    aistudio: AIStudio;
  }
...
// High-Fidelity Voice Synthesis Decoder
...
const playConciergeVoice = async (base64Data: string) => {
  try {
    ...
    console.warn("Voice playback interrupted.");
  }
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      setMessages([{
        id: 'welcome',
        role: Role.BOT,
        text: `Greetings. I am the ${config.name}. I provide expert consultation on ICT's Managed IT Services and Professional IT training programs. How may I direct your business today?`,
        timestamp: Date.now(),
      }]);
    }
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      // API CONFORMITY: Ensure history alternates User/Model and starts with USER.
      ...
      if (response.audioData && !isMuted) {
        playConciergeVoice(response.audioData);
      }
  ...
  return (
    <div className={`fixed bottom-0 right-0 z-[9999] transition-all duration-700 ease-[cubic-bezier(0.19,1,0.22,1)] flex flex-col items-end p-0 md:p-6 ${isMinimized ? 'w-auto' : 'w-full md:w-[480px] h-[100dvh] md:h-[85vh] max-h-[980px]'}`}>
      ...
      {/* Prime Interface */}
      <div className={`bg-white shadow-[0_100px_200px_-50px_rgba(0,0,0,0.8)] md:rounded-[4.5rem] border border-slate-200/50 flex flex-col overflow-hidden transition-all duration-1000 h-full w-full ${isMinimized ? 'scale-75 opacity-0 translate-y-40 pointer-events-none' : 'scale-100 opacity-100 translate-y-0'}`}>
        ...
        <header className="px-14 py-14 flex items-center justify-between border-b border-slate-50 bg-white/40 backdrop-blur-3xl sticky top-0 z-20">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[12px] text-slate-400 font-black uppercase tracking-[0.4em] leading-none">
                  {requiresAuth ? 'Handshake Required' : 'Obsidian Sync Online'}
                </span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setIsMinimized(true)} className="w-12 h-12 flex items-center justify-center text-slate-300 hover:text-slate-900">
        ...
        <main ref={scrollRef} className="flex-1 overflow-y-auto px-12 py-16 space-y-14 bg-[#fefeff]">
          {requiresAuth ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-10 animate-in fade-in zoom-in duration-1000">
              ...
              <p className="text-slate-500 text-[15px] font-medium leading-relaxed mb-10">To access the ICT Obsidian Core, please establish a secure handshake using your professional API credentials.</p>
              <button 
                onClick={handleAuth}
              ...
              <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="mt-8 text-[11px] font-black uppercase tracking-widest text-slate-300 hover:text-blue-600 transition-colors">Billing Documentation</a>
            </div>
          ) : (
            <>
              ...
              {messages.length === 1 && (
                <div className="grid grid-cols-1 gap-6 animate-in fade-in slide-in-from-bottom-16 duration-1000">
                  <p className="text-[12px] font-black uppercase tracking-[0.6em] text-slate-300 ml-5 mb-2">Verified Routing Shortcuts</p>
              ...
              {isLoading && (
                <div className="flex justify-start items-center gap-8">
                  ...
                  <p className="text-[13px] font-black text-slate-400 uppercase tracking-[0.5em] animate-pulse">Consulting Knowledge Matrix...</p>
                </div>
              )}
              ...
              {error && (
                <div className="mx-8 p-10 bg-rose-50 border border-rose-100 rounded-[3.5rem] text-rose-600 text-[14px] font-black text-center shadow-2xl shadow-rose-500/10 uppercase tracking-[0.2em] leading-relaxed">
                  {error}
                  <span className="text-[11px] opacity-70 block mt-4 font-bold">Synchronizing with secondary secure gateway.</span>
                </div>
              )}
            </>
          )}
      ...
      >
        <div className="flex items-center gap-8 bg-blue-600 pl-14 pr-10 py-8 rounded-[4.5rem] shadow-[0_60px_150px_-25px_rgba(37,99,235,0.9)] border border-white/20 hover:scale-105 active:scale-95 transition-all">
          <div className="flex flex-col">
            ...
            <span className="text-black text-[12px] font-black uppercase tracking-widest leading-none opacity-90">Secure Channel</span>
          </div>
